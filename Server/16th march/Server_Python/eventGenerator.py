...
class EventGen(threading.Thread):
    ...
    def __init__(self,threadID,group,re,signs):
        ...
        self.__signs = signs
    def run(self):
        #print(", Thread "+self.threadID+" =============>is running.")
        #print(self.__group)
        x_list=[]
        ...
        self.radixsort(x_list,self.__group,self.__signs)
        j=1
        i=0
        done=False
        result_list=[]
        ...
        for i in range(len(result_list)):
            ...
            for j in range(len(final_result[i])):
                ...
                #correspoding in small_sign
            #add in the third item small_sign
            self.radixsort(y_list,final_result[i],)
            #self.__realSign.append(small_sign)
        #self._signs=readSign
        result_list=[]
        ...
        for i in result_list:
            #this number means the minimum event size
            if(len(i)>2):
                #print(i)
                self.__events.append(i)

    def radixsort(self, aList,bList,cList ):
        RADIX = 10
        maxLength = False
        tmp , placement = -1, 1
        while not maxLength:
            ...
            buckets3= [list() for _ in range(RADIX)]
            bi=0
            ...
            for  i in aList:
                ...
                buckets3[int(tmp% RADIX)].append(clist[bi])
                bi+=1
